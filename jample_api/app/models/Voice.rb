...
class Voice  < ActiveRecord::Base
  ...
  def randomize_voice(subset_of_track_ids)
    begin
      ...
      # debugger
      current_audition  = self.auditions.create({track_id: track_id, })
      ...
      current_audition.stop_onset_index = 
        [(current_audition.start_onset_index + DURATION_IN_SLICES),
         (track_onset_array.size - 1)].min
  ...
  def back_one_audition
    ...
    current_audition = self.auditions.where("created_at < ?", self.current_audition.created_at ).order(created_at: :desc).first
  ...
  def forward_one_audition
    ...
    current_audition = self.auditions.where("created_at < ?", self.current_audition.created_at).order(created_at: :asc).first
    if current_audition.present?
      self.current_audition = current_audition
      self.save
      response = self.return_voice_hash()
