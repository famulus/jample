  ...
  grid: ->
    ...
    <div className="col-md-6">  
      <table className="table">
        <tr>
          { reversed_patches.slice(12,16).map (patch) => <Patch key={patch.id} currentPatch={@state.currentPatch } patch={patch}/>}
        </tr>
        <tr>
          { reversed_patches.slice(8,12).map (patch) => <Patch key={patch.id} currentPatch={@state.currentPatch } patch={patch}/>}
        </tr>
        <tr>
          { reversed_patches.slice(4,8).map (patch) => <Patch key={patch.id} currentPatch={@state.currentPatch } patch={patch}/>}
        </tr>
        <tr>
          { reversed_patches.slice(0,4).map (patch) => <Patch key={patch.id} currentPatch={@state.currentPatch } patch={patch}/>}
        </tr>
      </table>
    </div>
