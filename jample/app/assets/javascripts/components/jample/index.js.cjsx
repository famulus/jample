Jample = React.createClass
  ...
  componentDidMount: ->
    ...
    data = undefined
    @setState
      audios: @state.patch_set.patches.map (patch) ->
        new Howl(urls: [ "patch/pad_#{patch.patch_index}.mp3" ])
    ...
    noteOn = (noteNumber) =>
      ...
      patch_index = (noteNumber - 36)
      @state.audios[patch_index].play()
      @setState(currentPatch: (patch_index))

    noteOff = (noteNumber) =>
      # console.log 'noteOff: ' + noteNumber 
      patch_index = (noteNumber - 36)
      @state.audios[patch_index].stop()
      @setState(currentPatch: (patch_index))
      
    controller = (noteNumber,value) =>
      console.log 'controller: ' + noteNumber + ' value: ' + value 
      
    polyPressure = (noteNumber,value) =>
      # console.log 'polyPressure' + noteNumber + 'value' + value 
    ...
    onMIDIMessage = (ev) =>
      ...
      velocity = ev.data[2]
      # console.log 'cmd: ' + cmd + ' channel: ' + channel + ' noteNumber: ' + noteNumber + ' velocity: ' + velocity
      if channel == 9
        return
      if cmd == 8 or cmd == 9 and velocity == 0
        ...
        # poly aftertouch
        polyPressure noteNumber, noteNumber / 127
      else
